...
bool FeatureManager::addFeatureCheckParallax(
  ...
  for (auto &id_pts : image) {
    ...
    assert(id_pts.second[0].first ==
           0); // 判断该特征点是否左目camera_id==0 ，这里不应该先检查然后添加？
    ...
    if (it == feature.end()) {
      feature.push_back(
          FeaturePerId(feature_id, frame_count)); // 记录特征点id，当前帧索引
      feature.back().feature_per_frame.push_back(
          f_per_fra);    // 添加一个特征点数据

...
为了维持窗口大小，需要去除旧的帧添加新的帧，也就是边缘化
Marginalization。到底是删去最旧的帧（MARGIN_OLD）还是删去刚
刚进来窗口倒数第二帧(MARGIN_SECOND_NEW)
如果大于最小像素,则返回true */

/**
 * @brief
 * 把特征点放入feature的list容器中，即建立特征点和观测帧之间的联系，同时判断当前帧是否是关键帧，这决定了是边缘化滑动窗口中的次新帧还是最旧帧：
 * 1.该帧新的特征点很多，认为是关键帧
...
bool FeatureManager::addFeatureCheckParallax(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double td) {
    ...
    int parallax_num = 0;    // 满足某些条件的特征点数目
    last_track_num = 0;
    ...
    for (auto &id_pts : image) {
        ...
        assert(id_pts.second[0].first == 0); // 判断该特征点是否左目camera_id==0 ，这里不应该先检查然后添加？
        ...
        if (it == feature.end()) {
            feature.push_back(FeaturePerId(feature_id, frame_count)); // 记录特征点id，当前帧索引
            feature.back().feature_per_frame.push_back(f_per_fra);    // 添加一个特征点数据
            new_feature_num++;                                        // 当前帧新特征点数
        }
        // 如果该特征点已经在之前出现过，就在其FeaturePerFrame内增加此特征点在此帧的位置以及其他信息
        else if (it->feature_id == feature_id) {

            it->feature_per_frame.push_back(f_per_fra); // 添加一个特征点数据

            last_track_num++; // 当前帧老特征点数
            // 当前帧跟踪超过4次的特征点数
...
/**
 * SVD计算三角化点，得到特征点深度。原理参考：https://blog.csdn.net/qq_41904635/article/details/106092472
 * @param Pose0     位姿Tcw
 * @param Pose1     位姿Tcw
 * @param point0    归一化相机平面点
 * @param point1    归一化相机平面点
 * @param point_3d  output 三角化点在 世界坐标系下的坐标
 */
void FeatureManager::triangulatePoint(Eigen::Matrix<double, 3, 4> &Pose0, Eigen::Matrix<double, 3, 4> &Pose1, Eigen::Vector2d &point0, Eigen::Vector2d &point1,
...
/**
 * @brief 3d-2d PnP求解位姿，Twc
 * @param R [in,out]输入是w_R_cam，输出是w_R_cam  Rwc
 * @param P [in,out]输入是w_t_cam，输出是w_t_cam  twc
...
bool FeatureManager::solvePoseByPnP(Eigen::Matrix3d &R, Eigen::Vector3d &P, vector<cv::Point2f> &pts2D, vector<cv::Point3f> &pts3D) {
    ...
    bool pnp_succ;
    // 3d-2d Pnp 求解位姿。rvec和t是使坐标点从世界坐标系旋转到相机坐标系
...
void FeatureManager::initFramePoseByPnP(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[]) {
    ...
    if (frameCnt > 0) {
        ...
        vector<cv::Point3f> pts3D; // 世界坐标系下
        ...
        if (solvePoseByPnP(RCam, PCam, pts2D, pts3D)) {
            ...
            // Q.vec().transpose() << endl; cout << "frameCnt: " << frameCnt << " pnp P
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[]) {
    ...
    for (auto &it_per_id : feature) {
        ...
        if (STEREO && it_per_id.feature_per_frame[0].is_stereo) {
            ...
            //! SVD计算三角化出点的深度，point3d是世界坐标系下的坐标
...
/**
 * @brief
 * 删除feature变量中outlierIndex索引的外点。在滑窗优化之后计算重投影误差，剔除误差过大点
    ...
    for (auto it = feature.begin(), it_next = feature.begin(); it != feature.end(); it = it_next) {
        ...
        // 如果遍历到的当前特征点被移除的帧(第0帧)所看到，则交接管辖权
            ...
            // 说明当前特征点从原先第0帧中移除后，仍然至少被两帧所看到，表明它是一个有效特征点，下面进行管辖权交接（主要是更新一下特征点深度）
                ...
                // 转换过程: 特征点在第0帧的归一化坐标 ==>
                // 特征点在第0帧的相机坐标系下的坐标 ==> 特征点在世界坐标系下的坐标 ==>
                // 特征点在第1帧的相机坐标系下的坐标
...
/**
 * @brief
 * 还未成功初始化时的特征点处理：直接移除掉对应特征点，因为此时未初始化成功还没有深度
 */
...
void FeatureManager::removeFront(int frame_count) {
    ...
    for (auto it = feature.begin(), it_next = feature.begin(); it != feature.end(); it = it_next) {
        ...
        // 倒数第二帧能够观察到这个地图点
            ...
            // 如果能够观察到当前地图点的最后一帧索引 小于
            // 倒数第二，则当前地图点不能被倒数第二帧所看到

...
void Estimator::clearState() {
  ...
  f_manager.clearState(); // 对滑动窗口内所有特征点的管理函数 状态清空，开启
...
void Estimator::setParameter() {
  ...
  // 如果是多线程，就会启动多线程运行 processMeasurements
...
/**
 * 输入新的一帧的IMU数据供后端的非线性优化，每当订阅到新的imu话题数据就会执行该函数，当认为imu初始化完成后会调用fastPredictIMU
 * 1、更新位置P、旋转Q、速度V
 * 2、并发布到话题imu_propagate中，话题更新的频率和imu频率相同
 * @param t                     时间戳
...
void Estimator::inputIMU(double t, const Vector3d &linearAcceleration,
  ...
  if (solver_flag == NON_LINEAR) {
    mPropagate.lock();
    // IMU预积分：使用上一时刻的姿态进行快速的
    // IMU预积分，来预测当前帧位姿：latest_time，最新的旋转Q latest_Q、位置P
    // latest_P、速度V latest_V、latest_acc_0、latest_gyr_0
...
void Estimator::processMeasurements() {
  while (1) {
    ...
    // 存储一帧图像的特征点信息，key是时间戳，value是特征点数据值
    ...
    // featureBuf中有东西，开始进行以下处理
    // 有图像数据后，程序才发给跟踪器叫他产生feature，因此当featureBuf不等于空，所有的buffer，包括imu,图像，都不为空
    if (!featureBuf.empty()) {
      // 1. 取出当前帧识别的特征点信息: 时间戳, {feature_id，[camera_id
      // (0为左目，1为右目), x, y, z (去畸变的归一化相机平面坐标), pu, pv
      ...
      // 3. 获取
      // 前一帧与当前帧图像之间的IMU数据：时间戳，加速度计/陀螺仪数据提取出来，存入accVector、gyrVector中
      if (USE_IMU)
      ...
      if (USE_IMU) {
        ...
        if (!initFirstPoseFlag) {
          // 因为IMU不是水平放置，所以Z轴和{0,
          // 0, 1.0}对齐，通过对齐获得Rs[0]的初始位姿
          // 将初始时刻加速度方向对齐重力加速度方向，得到一个旋转矩阵，使得初始IMU的z轴指向重力加速度方向。
        ...
        // 遍历两帧间的IMU数据，对每一对相邻时刻的IMU数据，进行积分
      ...
      //! 5.
      //! 利用当前帧图像信息，进行后续的关键帧判断、初始化、非线性优化、边缘化、滑动窗口移动等操作
...
void Estimator::processImage(
  ...
  if (f_manager.addFeatureCheckParallax(frame_count, image, td)) {
    marginalization_flag =
        MARGIN_OLD; // 如果当前帧为关键帧，则边缘化marg滑动窗口中最旧的帧
                    // printf("keyframe\n");
  } else {
    marginalization_flag =
...
void Estimator::optimization() {
  ...
  /**
   * Step1.
   * 调用AddParameterBlock，显式添加待优化变量（类似于g2o中添加顶点），需要固定的顶点固定一下
   */
  ...
  // (1)
  // 添加先验残差，通过Marg的舒尔补操作，将被Marg部分的信息叠加到了保留变量的信息上
...
void Estimator::predictPtsInNextFrame() {
  ...
  nextT =
      curT * (prevT.inverse() * curT); // Twc * (Tlw * Twc = Tlc = Tcn) = Twn

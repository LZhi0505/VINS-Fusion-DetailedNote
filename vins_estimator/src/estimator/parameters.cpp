...
std::vector<Eigen::Matrix3d>
    RIC; // 队列，存储body_R_cam0矩阵，后续不断优化该参数
std::vector<Eigen::Vector3d>
    TIC; // 队列，存储body_t_cam0矩阵，后续不断优化该参数
...
void readParameters(std::string config_file) {
  ...
  ESTIMATE_TD = fsSettings
      ["estimate_td"]; // 是否在线获取相机与IMU的时间漂移，1表示两个传感器未同步，0表示两个传感器已同步
  if (ESTIMATE_TD)
    ROS_INFO_STREAM(

...
int main(int argc, char **argv) {
  ...
  */
  ...
  ros::Subscriber sub_imu =
      n.subscribe(IMU_TOPIC, 2000, imu_callback,
                  ros::TransportHints().tcpNoDelay()); // 订阅IMU消息
  ros::Subscriber sub_feature =
      n.subscribe("/feature_tracker/feature", 2000,
                  feature_callback); // 订阅feature_tracker所提供的跟踪光流点
  ros::Subscriber sub_img0 =
      n.subscribe(IMAGE0_TOPIC, 100, img0_callback); // 订阅左目图像
  ros::Subscriber sub_img1 =
      n.subscribe(IMAGE1_TOPIC, 100, img1_callback); // 订阅右目图像

  ros::Subscriber sub_restart =
      n.subscribe("/vins_restart", 100, restart_callback); // 订阅，重启节点
  ros::Subscriber sub_imu_switch = n.subscribe(
      "/vins_imu_switch", 100, imu_switch_callback); // 订阅，双目下，IMU开关
  ros::Subscriber sub_cam_switch = n.subscribe(
      "/vins_cam_switch", 100, cam_switch_callback); // 订阅，单双目切换

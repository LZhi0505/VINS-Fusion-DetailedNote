...
/**
 * 特征点集合 按 跟踪次数 从大到小重排序，
 * 并且将已跟踪到特征点，对应的 mask 位置周围 画半径MIN_DIST为 黑色 实心圆
*/
...
//    }
    ...
    if (prev_pts.size() > 0)
    {
        ...
        if(hasPrediction)
        {
            ...
            cv::calcOpticalFlowPyrLK(prev_img, cur_img, // 前一帧图片或金字塔，当前帧图片或金字塔
                                     ...
                                     cv::OPTFLOW_USE_INITIAL_FLOW); // 操作标志：OPTFLOW_USE_INITIAL_FLOW 使用初始估计，存储在nextPts中;如果未设置标志，则将prevPts复制到nextPts并将其视为初始估计
        ...
        if(FLOW_BACK)
        {
            ...
            for(size_t i = 0; i < status.size(); i++)
            {
                // 若正向、反向都匹配到了，且 用正向匹配点反向匹配回来的点 与 原始点距离<=0.5个像素，则最终认为跟踪成功
    ...
    if (1)
    {
        ...
        // 特征点集合 按 跟踪次数 从大到小重排序；并且将已跟踪到特征点，对应的 mask 位置周围 画半径MIN_DIST为 值为0(黑色)的实心圆
        // 意为 不再进行角点检测，避免重复
        setMask();
    ...
    // 当前帧左目特征点的 像素坐标 转为 归一化相机平面坐标（只包含x,y, 且带畸变校正）
    ...
    // 添加当前帧特征点
    // featureFrame: feature_id，[camera_id, x, y, z (归一化相机平面坐标), pu, pv (像素坐标), vx, vy (归一化相机平面移动速度)]
                                            ...
                                            map<int, cv::Point2f> &cur_id_pts, map<int, cv::Point2f> &prev_id_pts)
{
    ...
    if (!prev_id_pts.empty())
    {
        ...
        for (unsigned int i = 0; i < pts.size(); i++)
        {
            ...
            it = prev_id_pts.find(ids[i]);  // 可能找不到
            // 找到了，则计算该点在归一化相机平面上x、y方向的移动速度

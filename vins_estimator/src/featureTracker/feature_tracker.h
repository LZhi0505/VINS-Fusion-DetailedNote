...
public:
    ...
    void drawTrack(const cv::Mat &imLeft, const cv::Mat &imRight, vector<int> &curLeftIds, vector<cv::Point2f> &curLeftPts, vector<cv::Point2f> &curRightPts,
    ...
    // 上一帧左目、当前帧左目、当前帧右目中识别到的特征点 转换到 归一化相机坐标系的坐标（归一化相机x,y坐标，去畸变后的）
